...
// EnsureInvestigable checks that a ticket may enter the investigating state.
// Whoever is on shift may pick a replay up, so only the ticket grading has to be
// known at this point.
func (t *TriageTicket) EnsureInvestigable() error {
	if t.Severity < 1 || t.Severity > 5 {
		return apperr.Invalidf("ticket_severity_invalid", "处置单严重度必须在 1~5 之间")
	}
	return nil
