...
// CountAll reports how many campaigns the repository holds. It backs the list
// pagination envelope with a single cheap query.
func (r *campaignRepo) CountAll(ctx context.Context) (int, error) {
	var total int
	if err := r.q.QueryRowContext(ctx, `SELECT COUNT(1) FROM campaigns`).Scan(&total); err != nil {
		return 0, translate(err, "campaign_list_failed", "无法统计路测计划")
	}
	return total, nil
}
