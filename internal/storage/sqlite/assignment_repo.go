...
// CompletedByCampaign reads the completed shifts of a campaign in shift order.
// The limit keeps a settlement review from loading an unbounded result set.
func (r *assignmentRepo) CompletedByCampaign(
	ctx context.Context,
	campaignID int64,
	limit int,
	...
	return r.queryAssignments(ctx,
		`SELECT `+assignmentColumns+` FROM assignments
		 ...
		 ORDER BY shift_start ASC, id ASC LIMIT ?`,
		campaignID, string(domain.AssignmentCompleted), limit)
}
