...
func (r *captureRepo) AppendFrames(ctx context.Context, batchID int64, frames []*domain.CaptureFrame) error {
	...
	for _, frame := range frames {
		...
		if _, err := r.q.ExecContext(ctx, `
			INSERT INTO capture_frames (batch_id, sequence, sensor, payload_hash, quality_score,
			...
			return translate(err, "frame_append_failed", "帧序号重复或批次不存在")
		}
	}
	if _, err := r.q.ExecContext(ctx,
		`UPDATE capture_batches SET frame_count = (SELECT COUNT(1) FROM capture_frames WHERE batch_id = ?)
		 WHERE id = ?`, batchID, batchID); err != nil {
		return translate(err, "frame_count_sync_failed", "无法同步批次帧数")
	}
	return nil
}
...
// DropAllFrames voids every frame of a batch with one statement, which keeps a
// rejection cheap even for large uploads.
func (r *captureRepo) DropAllFrames(ctx context.Context, batchID int64, reason string) error {
	if _, err := r.q.ExecContext(ctx,
		`UPDATE capture_frames SET status = ?, reason = ? WHERE batch_id = ?`,
		string(domain.FrameDropped), reason, batchID); err != nil {
		return translate(err, "frame_status_update_failed", "无法作废采集帧")
	}
	return nil
}
