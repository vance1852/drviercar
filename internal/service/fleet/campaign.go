...
func (s *Service) ensureCampaignGuards(
	...
	case domain.CampaignCancelled:
		// A cancelled campaign no longer needs its cars, so hand every still
		// open shift's vehicle back to the depot instead of asking the operator
		// to terminate the shifts one by one first.
		if err := s.releasePlannedShiftVehicles(ctx, tx, campaign.ID); err != nil {
			return err
		}
	}
	return nil
...
func (s *Service) ListCampaigns(ctx context.Context, filter repository.CampaignFilter) (*CampaignPage, error) {
	...
	filter.Page = page
	items, total, err := s.store.Repos().Campaigns.List(ctx, filter)
	if err != nil {
		return nil, err
	}
