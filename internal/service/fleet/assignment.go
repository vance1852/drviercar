...
func (s *Service) AbortAssignment(
	...
	err := s.store.WithTx(ctx, func(ctx context.Context, tx *repository.Registry) error {
		...
		// A drive that is still open is reported for follow-up. It must not stop
		// the dispatcher from closing the shift administratively.
		if driveErr := s.ensureNoActiveDrive(ctx, tx, assignment.ID); driveErr != nil {
			s.logger.Warn(ctx, "shift terminated while a drive was still open", map[string]any{
				"assignment_id": assignment.ID,
				"reason":        reason,
				"detail":        driveErr.Error(),
			})
		}
		if err := s.recorder.Record(ctx, tx, audit.Entry{
			OperatorID: actor.OperatorID,
