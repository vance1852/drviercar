...
func (s *Service) CreateAssignment(
	...
	// Book the campaign mileage and the vehicle up front so the shift
	// transaction only has to write the assignment row and its audit event.
	campaign, err := s.store.Repos().Campaigns.ByID(ctx, input.CampaignID)
	if err != nil {
		return nil, err
	}
	...
	vehicle, err := s.store.Repos().Vehicles.ByID(ctx, input.VehicleID)
	if err != nil {
		return nil, err
	}
	...
	if err := s.store.Repos().Campaigns.ReserveKm(ctx, campaign.ID, input.PlannedKm); err != nil {
		return nil, err
	}
	if err := s.store.Repos().Vehicles.UpdateStatus(ctx, vehicle.ID, vehicle.Version,
		domain.VehicleReserved); err != nil {
		return nil, err
	}
