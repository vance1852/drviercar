...
// releasePlannedShiftVehicles hands the vehicles of every still open shift of a
// campaign back to the idle pool. It is used when a whole campaign is cancelled
// and its cars have to become available for other cities again.
func (s *Service) releasePlannedShiftVehicles(
	ctx context.Context,
	tx *repository.Registry,
	campaignID int64,
) error {
	shifts, total, err := tx.Assignments.List(ctx, repository.AssignmentFilter{
		CampaignID: campaignID,
		Statuses:   []domain.AssignmentStatus{domain.AssignmentPlanned, domain.AssignmentActive},
	...
	if total > len(shifts) {
		return apperr.Wrap(apperr.ErrPreconditionUnmet, apperr.KindPrecondition,
			"campaign_cancel_page_overflow",
			"路测计划下未完成排班过多，请先分批终止排班")
	}
	for _, shift := range shifts {
		vehicle, vehicleErr := tx.Vehicles.ByID(ctx, shift.VehicleID)
		if vehicleErr != nil {
			return vehicleErr
		}
		if vehicle.Status == domain.VehicleIdle {
...
func (s *Service) BatchAbortAssignments(
	...
	for _, assignmentID := range assignmentIDs {
		...
		if _, err := s.AbortAssignment(ctx, actor, assignmentID, reason); err != nil {
