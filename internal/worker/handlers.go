...
// PurgeSessions deletes sessions whose validity window elapsed.
func (m *Maintenance) PurgeSessions(ctx context.Context, _ *repository.Job) error {
	return m.Store.WithTx(ctx, func(ctx context.Context, tx *repository.Registry) error {
		_, err := tx.Sessions.DeleteExpired(ctx, m.Clock.Now())
		return err
	})
}
...
// permanentIfRefused marks a refusal that the domain already decided as a
// permanent job failure, so the queue does not keep retrying work that has
...
func (m *Maintenance) ArchiveBatch(ctx context.Context, job *repository.Job) error {
	...
	return permanentIfRefused(m.Store.WithTx(ctx, func(ctx context.Context, tx *repository.Registry) error {
