...
// RecordDecision appends an audit entry in its own transaction, before the
// business write is attempted. It is used for approval style actions where the
// decision itself must be on record even if the write has to be retried.
func (r *Recorder) RecordDecision(
	...
	return runner.WithTx(ctx, func(ctx context.Context, tx *repository.Registry) error {
		_, err := tx.Audit.Append(ctx, event)
		return err
	})
}
